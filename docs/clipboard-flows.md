...
关键设计：**emskin 内部只有一条 smithay dd 总线**。所有 world 都喂它，所有 world 都从它取。宿主侧和内部 X world 都是 smithay dd 的"接线盒"：

```
     外部宿主 world            ┌─────── emskin 内部 ────────┐
    (X11 / Wayland)            │                            │
     │       ▲                 │                            │
     │       │ ClipboardProxy  │     smithay dd             │
     ▼       │ / X11ClipProxy  │     (唯一总线)             │
 ┌────────────────────┐        │     ▲    ▲                 │
 │ ClipboardBackend   │◄──────►│─────┘    │                 │
 │ .set/.receive_from │ §1/2/3 │          │                 │
 └────────────────────┘        │          │ smithay         │
                               │          │ SelectionHandler│
                               │          │                 │
                               │  ┌───────┴──────┐          │
...
| # | 复制方 | 粘贴方 | 宿主模式 | 看图去这里 | origin | 焦点/serial |
|---|---|---|---|---|---|---|
| 1 | H-X11 外部 X | C-W | X11 | §1「外部 X 复制 → 内部 Wayland 粘贴」 | `Host` | 无 |
| 2 | C-W | H-X11 外部 X | X11 | §1「内部 Wayland 复制 → 外部 X 粘贴」 | `Wayland` | 无 |
| 3 | H-X11 外部 X | C-X | X11 | **§5.2** | `Host` | 无 |
| 4 | C-X | H-X11 外部 X | X11 | **§5.5** | `X11` | 无 |
| 5 | H-X11 外部 X ↔ H-X11 外部 X | — | X11 | X server 自处理，emskin 不参与 | n/a | — |
| 6 | H-W DC 外部 | C-W | W+DC | §2「外部复制 → 内部粘贴」 | `Host` | 无 |
| 7 | C-W | H-W DC 外部 | W+DC | §2「内部复制 → 外部粘贴」 | `Wayland` | 无 |
| 8 | H-W DC 外部 | C-X | W+DC | **§5.3** | `Host` | 无 |
| 9 | C-X | H-W DC 外部 | W+DC | **§5.6** | `X11` | 无 |
| 10 | H-W DC 外部 ↔ H-W DC 外部 | — | W+DC | 宿主处理，emskin 不参与 | n/a | — |
| 11 | H-W WD 外部 | C-W | W+WD | §3「外部复制 → 内部粘贴」 | `Host` | **emskin 要有焦点** |
| 12 | C-W | H-W WD 外部 | W+WD | §3「内部复制 → 外部粘贴」 | `Wayland` | **需要 latest_serial** |
| 13 | H-W WD 外部 | C-X | W+WD | **§5.4** | `Host` | **同 #11** |
| 14 | C-X | H-W WD 外部 | W+WD | **§5.7** | `X11` | **同 #12** |
| 15 | H-W WD 外部 ↔ H-W WD 外部 | — | W+WD | 宿主处理，emskin 不参与 | n/a | — |
| 16 | C-W | C-W | 无关 | §4 前提（smithay dd 自环） | `Wayland` | 无 |
| 17 | C-X | C-X | 无关 | **§5.8**（X server 自环） | `X11` | 无 |
| 18 | C-W | C-X | 无关 | §4「内部 wayland 复制 → 内部 X 粘贴」 | `Wayland` | 无 |
| 19 | C-X | C-W | 无关 | §4「内部 X 复制 → 内部 wayland 粘贴」 | `X11` | 无 |

**关键观察**：

- **内部桥（#16~#19）对宿主完全不敏感**：三种宿主下行为一致。
...
- **WD 独有的约束只影响 4 条带宿主段的 cell（#11~#14）**，纯内部场景（#16~#19）与 DC/X11 完全相同。
...
- **WD 的 serial 在跨层场景里依然是 `None` 就静默放弃**，没有 "降级走 DC" 之类的回退 — 运行时只会选其中一条宿主段。
