...
- Elisp workspace switch: `emskin--resync-visible-apps` bypasses sync-all's change detection to force re-sending geometry (triggering migration). `other-frame` advice is `:around` (not `:before`) — must run resync AFTER frame switch so apps migrate to the new active workspace
